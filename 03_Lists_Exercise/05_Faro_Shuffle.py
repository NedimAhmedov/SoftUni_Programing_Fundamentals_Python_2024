deck_of_cards = input().split()
count_of_shuffles = int(input())

for shuffle in range(count_of_shuffles):
    middle_of_the_deck = len(deck_of_cards) // 2
    left_part = deck_of_cards[:middle_of_the_deck]  # слайсинг - от начало до среден елемент(middle_of_the_deck)
    right_part = deck_of_cards[middle_of_the_deck:]  # слайсинг - от среден елемент (middle_of_the_deck) до край
    deck_after_shuffle = []
    for card_index in range(len(left_part)):
        deck_after_shuffle.append(left_part[card_index])
        deck_after_shuffle.append(right_part[card_index])
    deck_of_cards = deck_after_shuffle
print(deck_of_cards)