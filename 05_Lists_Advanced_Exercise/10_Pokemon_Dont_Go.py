def manipulate_distance(distance_list, sum_removed_elements, current_index):
    ...
    for each_index in range(len(distance_list)):
        if distance_list[each_index] <= removed_element:
            distance_list[each_index] += removed_element
        else:
            distance_list[each_index] -= removed_element
...
while distance:  # while still have items in list
    index = int(input())
